...
static userTimes(...fildes) { //userTimes data file
  return fs.
  readFile("./src/database/userTimes.json",  'utf8', (err, data) => {
  })
...
static  async As(cl) {   //기존 로그인 id, psword 를 덮어쓰기 즉 초기화 해줌
  ...
  const userTimes = await this.userTimes()
 ...
 if(cl.noName !== "username") {
  ...
  var usertimes = userTimes.filter(function (addSave) { return addSave.id === cl.id });
 

     function  delivery() { // 신규등록 다음 신규등시 실행 되는 함수
      return new Promise( (resolve, reject) => {
      setTimeout(() => {
      if(resolve) {
           
         
        if(newUserGoodsKinds[0] === undefined &&  usertimes[0] === undefined  ) {
          ...
          const comb = phonSub + pswsub //문열기 비번 조합 comb
                  userGoodsKinds.push({"id": users.id ,"name":users.name, "cdId": comb,
                              ...
                              "loginStart": [], "logoutEnd" : [],
                               "koko":[], "goods":["N"]})
                               userTimes.push({"id": users.id ,"name":users.name, "cdId": comb,
                               "phon":users.phon, "wonset":[] ,
                               "UseTime":[] , "goodsName":[],
                               "benchName":[],"expiryName":[],
                               "loginStart": [], "logoutEnd" : [],
...
// 자리 선정하는 함수 경로 bench.js "/"}
   ...
   var userRemove = UserTimes.filter(function (addSave) { return addSave.id === ab.id});
    ...
    function  delivery() {
      ...
      if(resolve) {
        if(add.setGoods === "fixedType" || add.setGoods === "daysType" ) {
           ...
           fs.writeFile("./src/database/userTimes.json",JSON.stringify(userGoodsKinds))
...
static async locaUser(client) { //logout 버튼 시 초기감 만들어줌
  ...
  static async logouttimes(client) { //logout 버튼 다음으로 퇴실 처리시 실행하는 함수 logout 시간을 담아줌
    ...
    const modiFY = await this.objectsave()
    var kend = modiFY.filter(function (addSave) { return addSave.id === client.id });
    var userTimes = UserTimes.filter(function (addSave) { return addSave.id === client.id })
    //  Number(kend[0].loginStart[client.index])
  
     const dateB = moment(`${nowTime}`);
   
     const login =   kend[0].loginStart[client.index] //GoddsKinds
     const login_1 =   UserTimes[0].loginStart[client.index] //useTimes
     
     const nexetTime= dateB.diff(login, 'minute') //GoodsKinds
     const nexetTime_1= dateB.diff(login_1, 'minute') //useTimes
     console.log(nexetTime, kend[0].UseTime[client.index],"kkkk")
              
      
     const Usetime = kend[0].UseTime[client.index] - nexetTime   //GoodsInds
     const usetime = UserTimes[0].UseTime[client.index] - nexetTime //userTimes
     const save = [] 
    function  de() { 
      return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        if(userTimes[0].UseTime[client.index]>0 && userTimes[0].wonset[client.index] !== undefined  && client.adminId !== "ADMIN") {
       ...
       fs.writeFile("./src/adminSetKinds/adminNext.json", JSON.stringify(adminNe), (err) => {
        ...
        userTimes[0].logoutEnd[client.index] = nowTime
        userTimes[0].UseTime[client.index] = usetime
        userTimes[0].wonset[client.index] = ""
        userTimes[0].loginStart[client.index] = ""
        
           fs.writeFile("./src/database/userGoodsKinds.json",JSON.stringify(UserTimes))
...
static async adminGoods(userIds) { //admin 에서 상품권 지급
  ...
  var Usertiems = UserTimes.filter(function (addsave) { return addsave.id === userIds.id});
 
    const nowTime =  moment().format('yyyy-MM-DD hh:mm')
  if(Usertiems[0].loginStart[userIds.sessionIndexof] !== "" && Usertiems[0].wonset[userIds.sessionIndexof] === "" && userIds.sessionIndexof !== null){
       Usertiems[0].loginStart[userIds.sessionIndexof] = nowTime
  ...
  }else if(Usertiems[0].loginStart[userIds.sessionIndexof] !== "" && Usertiems[0].wonset[userIds.sessionIndexof] !== "" && userIds.sessionIndexof !== null){
      // console.log(UserTimes.wonset[userIds.sessionIndexof])
  return { success: false, mag : `현재 상품에 ${UserTimes[0].wonset[userIds.sessionIndexof]} 자리를 사용하고 계습니다.` , wonset:UserTimes.wonset[userIds.sessionIndexof]}

  }else if(UserTimes[0].loginStart[userIds.sessionIndexof] === "") {
    UserTimes[0].loginStart[userIds.sessionIndexof] = nowTime
    fs.writeFile("./src/database/userTimes.json", JSON.stringify(UserTimes) , 'utf8' , (err) => {
  if (err) throw err;
     console.log("err") 
    ...
    return { success: false, mag : " 상품을  먼져 선택(클릭) 해주세요." , wonset:UserTimes.wonset[userIds.sessionIndexof]}
  }
